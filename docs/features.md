...
- **市场总开关**：遇到漏洞等紧急情况可一键停市（二次确认）——交易类入口（精灵市场/物品市场/拍卖场/求购单）与喵喵银行的借款入口随即置灰、点击给提示；**关市只拦交易**：设置与交易历史照常可用，喵喵银行的存款/取款/还款与卡片申请也不受影响，但待领取、领余额与下架要等市场恢复（资产不会丢失）；OP / 管理员仍可进管理员面板清理
...
- **极限特训显示**：特训后的个体值显示为「真实值（特训值）」，判定按有效值

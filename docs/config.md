...
| 字段 | 默认 | 说明 |
|---|---|---|
| `currency.cobblemonEconomy` | ~~自动~~ **已停用** | ⚠ 当前版本**暂时停用**（恒为 `false`，写 `true` 也不生效）：Cobblemon 1.8 与 Cobblemon Economy 不兼容，详见货币系统页。原本：是否优先使用 Cobblemon Economy 的货币 API；**仅 Fabric 平台生效**（该模组无 NeoForge 版，NeoForge 上此开关恒被忽略） |

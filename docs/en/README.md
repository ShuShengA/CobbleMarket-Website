...
Virtual currency is optional: [CobbleDollars](https://modrinth.com/mod/cobbledollars) or [Impactor](https://modrinth.com/mod/impactor). Without one, the market uses an item currency (diamond by default). **[Cobblemon Economy](https://modrinth.com/mod/cobblemon-economy) is incompatible with Cobblemon 1.8+** (it crashes the server whenever a player obtains or levels up a Pokémon) — see [Currency](/en/currency) for details.

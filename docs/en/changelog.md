...
> This page lists the changes for **every version**; the latest is **1.1.0**, followed by all previously released versions below.
...
- **Search box placeholder in the Auction Hall's Item and Mine tabs didn't follow the active tab**: returning from the auction listing screen or resizing the window reverted it to "Species name..."; it now always shows the current tab's hint. Item search hints are now consistently "Search items..." across the Auction Hall, blacklist and price-limit screens
- **Long item names squeezed out the count in Auction Hall and admin auction rows**: an over-long name truncated the "×N" suffix along with it, hiding how many are for sale — the count now always shows in full and the name truncates on its own

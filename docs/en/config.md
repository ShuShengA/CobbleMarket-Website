...
|---|---|---|
| `currency.cobblemonEconomy` | auto | ⚠ **Incompatible with Cobblemon 1.8+** (enabling it crashes the server when a player picks a starter Pokémon; a startup warning is logged). Prefer Cobblemon Economy's currency API. **Fabric only** — no NeoForge build, so this switch is always ignored on NeoForge |

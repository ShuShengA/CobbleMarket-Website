...
2. **昵称以 `query-order` 的 `user_name` 为准**，`query-sponsor` 的 `user.name` 只作兜底 —— 赞助者改名保存成功、**爱发电自己的界面却可能不刷新**，此时 sponsor 仍返回旧的默认名（实例：同一人 sponsor 给 `爱发电用户_34a08`、order 给 `美女淼`；用户拍板显示后者）。
